...
float4 BloomHorizontalPassFragment (Varyings input) : SV_TARGET {
	...
	for (int i = 0; i < 9; i++) {
		float offset = offsets[i] * 2.0 * GetSourceTexelSize().x; // downsample
...
float4 BloomVerticalPassFragment (Varyings input) : SV_TARGET {
	...
	for (int i = 0; i < 5; i++) {
		float offset = offsets[i] * GetSourceTexelSize().y; // no downsample
...
float4 BloomPrefilterFirefliesPassFragment (Varyings input) : SV_TARGET {
	float3 color = 0.0;
	float weightSum = 0.0;
	float2 offsets[] = {float2(-1.0, -1.0), float2(-1.0, 1.0), float2(0.0, 0.0),
...
float3 ColorGradingSplitToning (float3 color, bool useACES) {
	...
	float3 hightlights = lerp(0.5, _SplitToningHighlights.rgb, t);
...
float3 ColorGradingChannelMixer (float3 color) {
	return mul(
		float3x3(_ChannelMixerRed.rgb, _ChannelMixerGreen.rgb, _ChannelMixerBlue.rgb), color
	);
}
...
float3 ColorGradingShadowsMidtonesHighlights (float3 color, bool useACES) {
	...
	return color * (_SmhShadows.rgb * shadowsWeight + _SmhMidtones.rgb * midtonesWeight + _SmhHighlights.rgb * highlightsWeight); 
}
...
float3 GetColorGradingLUT (float2 uv, bool useACES = false) {
	float3 color = GetLutStripValue(uv, _ColorGradingLUTParameters);
	return ColorGrade(_ColorGradingLUTInLogC ? LogCToLinear(color) : color, useACES);
}

float4 ColorGradingNonePassFragment (Varyings input) : SV_TARGET {
	float3 color = GetColorGradingLUT(input.screenUV);
	return float4(color, 1.0);
}


float4 ColorGradingACESPassFragment (Varyings input) : SV_TARGET {
	float3 color = GetColorGradingLUT(input.screenUV, true);
...
float4 ApplyColorGradingWithLumaPassFragment (Varyings input) : SV_TARGET {
	float4 color = GetSource(input.screenUV);
	color.rgb = ApplyColorGradingLUT(color.rgb);
	color.a = sqrt(Luminance(color.rgb)); // gamma
	return color; 
}
